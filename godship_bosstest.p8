...
function _draw()
	...
	for unit in all(hudunit) do
		if (unit.axis == 'x') then
			drawhudunit(unit, offsetx, 0)
			offsetx += 8
		else
			drawhudunit(unit, 0, offsety)
			offsety += 8
		end
	end

	-- killboard hud
	local killsum = killboard.walker + killboard.jumper + killboard.flyer
	for i = 1, flr(killsum / 32) do
		spr(1, cam.x, cam.y + 15*8 - 6*i, 1, 1, false)
	end
	for i = 1, killsum - 32*flr(killsum / 32) do
		spr(21, cam.x + 15*8, cam.y + 15*8 - 4*i, 1, 1, false)
	end
...
function updatebossroom()
	...
	elseif (bossroomstate == 1) then
		...
		elseif (killboard.walker <= 1) then
			bossroomstate = 3
		end
...
function callbackcollisionparticuleunit(unit1, unit2)
	...
	if (particule.life > 0 and particule.damage > 0) then
		...
		if (unit.healthpoint <= 0) then
			unit.visible = false
			unit.state = unit_state_dead
			if (unit.type == ennemy_walker) then killboard.walker += 1
			elseif (unit.type == ennemy_jumper) then killboard.jumper += 1
			elseif (unit.type == ennemy_flyer) then killboard.flyer += 1
			end
